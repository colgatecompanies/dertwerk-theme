...
export interface ThemeChoice { palette: Palette; mode: Mode }

export const DEFAULT_THEME: ThemeChoice = { palette: 'field', mode: 'dark' }
...
function isDertwerkHost(hostname: string): boolean {
  return hostname === 'dertwerk.com' || hostname.endsWith('.dertwerk.com')
}

function parse(raw: string | null | undefined): ThemeChoice | null {
  if (!raw) return null
  const [p, m] = raw.split(':')
  if ((p !== 'gold' && p !== 'field') || (m !== 'light' && m !== 'dark')) return null
  return { palette: p, mode: m }
}

export function readCookie(): ThemeChoice | null {
...
export function writeCookie(choice: ThemeChoice): void {
 ...
 */
export function bootstrapTheme(): ThemeChoice {
 ...
 */
function migrateLegacyLocalStorage(): ThemeChoice | null {
  ...
  const choice: ThemeChoice = { palette: DEFAULT_THEME.palette, mode: legacy }
  writeCookie(choice)
  try {
...
export function adoptServerTheme(me: {
  ...
  const fromServer = parse(
    me.ui_palette && me.ui_theme ? `${me.ui_palette}:${me.ui_theme}` : null,
  )
